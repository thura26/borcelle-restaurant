...
**Customer အတွက်**
- GSAP animation နဲ့ ဆွဲဆောင်မှုရှိသော Landing Page — Hero, Gallery, Cuisine Grid, Ticker
- Menu ၆ ခု (SPICY NOODLES / BIBIMBAP / K-BBQ / FRIED / SOUPS / DRINKS) — ၇၂ မျိုးကျော်, MMK ဈေးနှုန်း
- Reservation စနစ် — ရက်စွဲ/အချိန်/လူဦးရေ ရွေးပြီး booking
- Cart & Checkout — ပစ္စည်းထည့်ခြင်း၊ promo code၊ မှာယူခြင်း
- Login/Register + Role-based (admin/user)
- Account စာမျက်နှာ — မှာယူမှုနဲ့ reservation မှတ်တမ်း ကြည့်ရန်
- Toast, Theme, Audit စသည့် UX provider များ

**Admin Dashboard** (`/admin`)
- DashboardOverview — recharts နဲ့ စာရင်းဇယား
- Products / Categories / Menu — CRUD, CSV import/export, AI Menu Generator
- Orders / Reservations — အခြေအနေပြင်ဆင်ခြင်း
- Promos, Users, Settings, Audit Log

### 🛠️ နည်းပညာများ

React 19 + Vite 8 + TypeScript 6 + Tailwind CSS 3 + GSAP 3 + lucide-react + recharts + React Router 7
...
npm run preview  # build ကို စမ်းကြည့်ရန်
```

Node.js 18+ လိုအပ်ပါတယ်။ `.env` မလိုသေးပါ — mock data နဲ့ပဲ အလုပ်လုပ်ပါတယ်။

### 📁 ဖိုင်ဖွဲ့စည်းပုံ

`src/components` — UI အပိုင်းများ, `src/pages/admin` — Admin စာမျက်နှာများ, `src/context` — ၁၂ ခုသော Context Provider, `src/lib/data.ts` — Menu data အားလုံး

Menu ပြင်ချင်ရင် `src/lib/data.ts` တစ်ဖိုင်တည်းပြင်ရုံပဲ ရပါတယ်။

### 🎨 Design

အဓိကအရောင် `primary #C1272E` (အနီ), Background `#FFFBF5`, Font `Poppins` — `tailwind.config.js` နဲ့ `src/index.css` မှာ ပြင်နိုင်ပါတယ်။

---

## 🤝 Contributing

PR များ ကြိုဆိုပါတယ်!
...
MIT — `LICENSE` မရှိသေးလျှင် `MIT License` ဖြင့် အသုံးပြုနိုင်ပါသည်။

---

<p align="center">
  Made with ❤️ for Borcelle Restaurant — Yangon, Since 2019<br/>
  <sub>Bringing The Fire Of Korean Spice To Yangon</sub>
</p>
